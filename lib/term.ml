...
module DeBruijn = struct
  ...
  let get_bound_var env name = List.find_opt (fun (named, _) -> named = name) env.vars
  ...
  let%expect_test "Test if the conversion of normal lambda to debruijn indices works" =
    let term =
      Abstr ("x", Apply (Abstr ("y", Var "x"), Abstr ("y", Abstr ("x", Var "x"))))
    in
    let env = new_env () in
